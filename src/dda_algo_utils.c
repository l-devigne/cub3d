...
void	init_step_map_grid(t_ray *ray)
{
	...
	{
		ray->step_map.x = -1;
		ray->side_dist.x = (ray->ray_origin.x - ray->ray_map_origin_x)
			* ray->dist_next_cel.x;
	}
	else
	{
		ray->step_map.x = +1;
		ray->side_dist.x = (ray->ray_map_origin_x + 1.0 - ray->ray_origin.x)
			* ray->dist_next_cel.x;
	}
	if (ray->ray_dir->y < 0)
	{
		ray->step_map.y = -1;
		ray->side_dist.y = (ray->ray_origin.y - ray->ray_map_origin_y)
			* ray->dist_next_cel.y;
	}
	else
	{
		ray->step_map.y = +1;
		ray->side_dist.y = (ray->ray_map_origin_y + 1.0 - ray->ray_origin.y)
			* ray->dist_next_cel.y;
	}
}
