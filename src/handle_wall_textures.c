...
/*   Updated: 2025/08/29 19:23:57 by meruem           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void texture_handling(t_data *data, char *texture_path, int side, t_ray ray, float perpWallDist)
{
    ...
    tex_img = mlx_xpm_file_to_image(data->mlx, texture_path, &data->text->width, &data->text->height);
    if (!tex_img)
    {
    ...
    data->text->tex_addr = mlx_get_data_addr(tex_img, &data->text->bpp, &data->text->line_len, &data->text->endian);

    double wallX; // exact hit coordinate on the wall
    if (side == 0)
    ...
    data->text->texX = (int)(wallX * (double)data->text->width);
